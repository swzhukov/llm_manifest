...
Source: QW | Date: 2026-05-13 | Fragments: 35 | ID: 041f2751-5b57-43ba-8579-eb3601531746
...
def extract_pdf_text(file_path):
    """Извлекает текст из PDF (PyPDF2 + pdfplumber). Если нет текста и включён OCR, конвертирует страницы в изображения и OCR."""
    ...
    if text.strip():
        return text.strip()
    # Если текст не извлечён и разрешён OCR
    if ENABLE_OCR:
        print(f"  [OCR] PDF без текста, применяю OCR к страницам: {file_path}")
        try:
            from pdf2image import convert_from_path   # нужно установить? Но это может быть недоступно. Использую альтернативный подход через pdftoppm (из poppler)
            # Установка poppler-utils
            !apt-get install -y -qq poppler-utils > /dev/null 2>&1
            import subprocess, tempfile, os
            with tempfile.TemporaryDirectory() as tmpdir:
                subprocess.run(["pdftoppm", "-png", "-r", "300", file_path, os.path.join(tmpdir, "page")], check=True)
                images = sorted(Path(tmpdir).glob("page-*.png"))
                ocr_text = []
                for img_path in images:
                    t = ocr_image(str(img_path))
                    if t:
...
def extract_pdf_text(file_path):
    ...
    if text.strip():
        return text.strip()
    if ENABLE_OCR:
        print(f"  [OCR] PDF без текста, применяю OCR...")
        try:
            with tempfile.TemporaryDirectory() as tmpdir:
                subprocess.run(["pdftoppm", "-png", "-r", "300", file_path, os.path.join(tmpdir, "page")], check=True)
                images = sorted(Path(tmpdir).glob("page-*.png"))
...
def extract_document_content(file_path, media_type):
    ext = Path(file_path).suffix.lower()
    if ext in [".mp3", ".wav", ".ogg", ".m4a", ".flac", ".opus", ".mp4", ".webm", ".mkv", ".avi", ".mov"]:
        return None
    try:
        if ext == ".pptx":   return extract_pptx_text(file_path)
        if ext == ".docx":   return extract_docx_text(file_path)
        if ext == ".pdf":    return extract_pdf_text(file_path)
        if ext in [".xlsx", ".xls"]: return extract_xlsx_text(file_path)
...
def extract_document_content(file_path, media_type):
    ...
    try:
        if ext == ".pptx":   return extract_pptx_text(file_path)
        if ext == ".docx":   return extract_docx_text(file_path)
        if ext == ".pdf":    return extract_pdf_text(
...
def extract_pptx(path):
    ...
    for i, slide in enumerate(prs.slides, 1):
        ...
        if slide.has_notes_slide:
            notes = slide.notes_slide.notes_text_frame.text.strip()
            if notes: slide_parts.append(f"Заметки: {notes}")
        if slide_parts: parts.append(f"### Слайд {i}\n" + "\n".join(slide_parts))
    return "\n\n".join(parts)
...
def extract_pdf(path):
    ...
    if text.strip(): return text.strip()
    if ENABLE_OCR:
        print(f"  [OCR] PDF без текста: {path}")
        try:
            with tempfile.TemporaryDirectory() as td:
                subprocess.run(["pdftoppm","-png","-r","300",path,os.path.join(td,"page")], check=True)
                ocr_parts = []
...
def extract_xlsx(path):
    try:
        ...
        for name in wb.sheetnames:
            ...
            if rows: parts.append(f"### Лист: {name}\n" + "\n".join(rows))
        wb.close()
        return "\n\n".join(parts)
    except Exception as e:
...
def extract_document_content(file_path):
    ext = Path(file_path).suffix.lower()
    if ext in [".mp3",".wav",".ogg",".m4a",".flac",".opus",".mp4",".webm",".mkv",".avi",".mov"]: return None
    try:
        if ext == ".pptx": return extract_pptx(file_path)
        if ext == ".docx": return extract_docx(file_path)
        if ext == ".pdf":  return extract_pdf(file_path)
        if ext in [".xlsx",".xls"]: return extract_xlsx(file_path)
        if ext in [".txt",".md",".csv",".json",".xml",".html"]:
...
def transcribe_with_newton(file_path, tmpdir):
    ...
    try:
        ...
        try:
            ...
            # Не JSON, значит просто текст
            return output

    except subprocess.CalledProcessError as e:
        print(f"  [!] ffmpeg error: {e.stderr[:200] if e.stderr else str(e)[:200]}")
        return None
    except Exception as e:
        print(f"  [!] Transcribe error: {e}")
        return None
...
def transcribe_with_newton(file_path, tmpdir):
    try:
        ...
        # Newton возвращает JSON или простой текст
        try:
            data = json.loads(output)
            if isinstance(data, dict):
                for field in ["text", "transcription", "transcript", "result"]:
...
Этот промт побуждает LLM провести многоуровневый, осмысленный анализ, а не поверхностную выжимку, и содержит полный контекст нашей предварительной работы.
...
4. Если файл существует и не пуст — использовать его содержимое как расшифровку.
5. Если результат — JSON (может содержать поле `text` или `transcription`), корректно извлечь текст.
...
- **Отладка**: если `DEBUG_MODE=True`, вывести все уникаль
...
def transcribe_with_newton(file_path, tmpdir, diarize=False):
    ...
    try:
        ...
        if os.path.isfile(result_file):
            ...
            if text:
                return text
        print(f"  [!] audio_result.txt не найден для {Path(file_path).name}")
        return None
    except subprocess.CalledProcessError as e:
        print(f"  [!] ffmpeg/Newton error: {e.stderr[:200] if e.stderr else str(e)[:200]}")
        return None
...
def extract_pptx(path):
    """Извлечение текста из PPTX, включая таблицы и заметки."""
    try:
        prs = Presentation(path)
        parts = []
        for i, slide in enumerate(prs.slides, 1):
...
def extract_pdf(path):
    """Извлечение текста из PDF (PyPDF2 + pdfplumber + OCR при необходимости)."""
    text = ""
    try:
...
def transcribe_with_newton(file_path, tmpdir, diarize=False, num_speakers=None):
    ...
    try:
        ...
        subprocess.run(cmd, capture_output=True, text=True, timeout=600, cwd=tmpdir)
...
def extract_pptx(path):
    """
    Извлечение текста из PPTX, включая таблицы, заметки и группы фигур.
    """
    def extract_shape_text(shape):
        """Рекурсивно извлекает текст из фигуры (в том числе GroupShape)."""
        text_parts = []
        if shape.has_text_frame:
        ...
        if shape.has_table:
            ...
            for row in shape.table.rows:
                ...
                rows.append("| " + " | ".join(cells) + " |")
            text_parts.append("\n".join(rows))
        # Рекурсивно обрабатываем дочерние элементы (GroupShape)
    ...
    try:
        ...
        for i, slide in enumerate(prs.slides, 1):
            slide_parts = []
            for shape in slide.shapes:
                slide_parts.extend(extract_shape_text(shape))
            if slide.has_notes_slide:
                notes = slide.notes_slide.notes_text_frame.text.strip()
                if notes:
                    slide_parts.append(f"Заметки: {notes}")
...
1. **Краткий анализ:** правильный ли мы выбрали подход, какие альтернативы можно рассмотреть, что улучшить.
2. **Полностью готовый Python-скрипт** для одной ячейки Colab, который можно скопировать и запустить. Код должен быть подробно прокомментирован на русском, учитывать все перечисленные требования и уроки, полученные в процессе разработки.
```

Этот промт специально составлен так, чтобы модель сначала задумалась о корректности всего подхода, предложила улучшения, а затем реализовала их в идеальном скрипте, который мы так долго отлаживали.
